...
def calc_secondary(av,b):
    ...
    # Get gas properties from application variables
    cv = av['cv']; cp = av['cp']; gam = av['gam']; R = av['rgas'];

    # Reference state for entropy
    Pref = 101325.0; Tref = 288.15
    ...
    b['t'] = (b['roe'] / b['ro'] - 0.5 * b['v']**2) / cv
    b['mach'] = b['v'] / (gam * R * b['t'])**0.5
    b['tstag'] = b['t'] * (1 + 0.5 * (gam-1) * b['mach']**2)

    # Pressures
    b['p'] = b['ro'] * R * b['t']
    b['pstag'] = b['p'] * (1 + 0.5 * (gam-1) * b['mach']**2) ** (gam / (gam-1))

    # Angles
    b['alpha'] = 180 * np.arctan2(b['vy'],b['vx']) / np.pi

    # Entropy
    b['s'] = cp * np.log(b['tstag'] / Tref) - R * np.log(b['pstag'] / Pref)
    b['sfunc'] = np.exp(-b['s'] / R)
...
def cut_j(b,j):
    # Take a structured cut along a "j = const" line, this is done using Python
    # indexing that starts from 0
    
    # Loop over all variables in dict and extract at given j
    c = {}
    for var in b:
        if isinstance(b[var],np.ndarray):
...
def mass_av(c,prop):
    ...
    p = np.sum(face_av(c[prop]) * mass) / mass_total 
...
def colorbar(mappable,name):
    ...
    # Remove the offset
    if hasattr(cbar.formatter,'set_useOffset') == 1:
