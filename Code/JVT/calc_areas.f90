...
!     INSERT
      real :: l_i_min, l_j_min
      real, dimension(g%ni-1,g%nj-1) :: dx_1, dx_2, dy_1, dy_2
...
!     INSERT
      dx_1 = g%x(2:ni,2:nj) - g%x(1:ni-1,1:nj-1) 
      dx_2 = g%x(1:ni-1,2:nj) - g%x(2:ni,1:nj-1)
      dy_1 = g%y(2:ni,2:nj) - g%y(1:ni-1,1:nj-1) 
      dy_2 = g%y(1:ni-1,2:nj) - g%y(2:ni,1:nj-1)
      g%area = 0.5 * (dx_1 * dy_2 - dx_2 * dy_1)
...
!     INSERT
      l_i_min = minval(hypot(g%lx_i,g%ly_i))
      l_j_min = minval(hypot(g%lx_j,g%ly_j))
      g%l_min = min(l_i_min,l_j_min) 
!
